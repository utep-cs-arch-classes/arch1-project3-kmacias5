 ...
 *  Adapted from RobG's EduKit
 */
void drawChar5x6(u_char rcol, u_char rrow, char c, 
     u_int fgColorBGR, u_int bgColorBGR) 
{
  u_char col = 0;
  u_char row = 0;
  ...
  while (row < 8) {
    while (col < 5) {
      u_int colorBGR = (font_5x6[oc][col] & bit) ? fgColorBGR : bgColorBGR;
      lcd_writeColor(colorBGR);
      col++;
    }
    col = 0;
 ...
 *  \param bgColorBGR Background color in BGR
 */
void drawString5x6(u_char col, u_char row, char *string,
		u_int fgColorBGR, u_int bgColorBGR)
{
  u_char cols = col;
  while (*string) {
    drawChar5x6(cols, row, *string++, fgColorBGR, bgColorBGR);
    cols += 6;
  }
}
